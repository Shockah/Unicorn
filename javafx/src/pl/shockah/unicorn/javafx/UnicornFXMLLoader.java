...
import static javafx.fxml.FXMLLoader.BINDING_EXPRESSION_SUFFIX;
import static javafx.fxml.FXMLLoader.BI_DIRECTIONAL_BINDING_PREFIX;
import static javafx.fxml.FXMLLoader.CHANGE_EVENT_HANDLER_SUFFIX;
import static javafx.fxml.FXMLLoader.CONTROLLER_KEYWORD;
import static javafx.fxml.FXMLLoader.CONTROLLER_METHOD_PREFIX;
import static javafx.fxml.FXMLLoader.CONTROLLER_SUFFIX;
import static javafx.fxml.FXMLLoader.COPY_SOURCE_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.COPY_TAG;
import static javafx.fxml.FXMLLoader.DEFAULT_CHARSET_NAME;
import static javafx.fxml.FXMLLoader.DEFINE_TAG;
import static javafx.fxml.FXMLLoader.ESCAPE_PREFIX;
import static javafx.fxml.FXMLLoader.EVENT_HANDLER_PREFIX;
import static javafx.fxml.FXMLLoader.EVENT_KEY;
import static javafx.fxml.FXMLLoader.EXPRESSION_PREFIX;
import static javafx.fxml.FXMLLoader.FX_CONSTANT_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.FX_CONTROLLER_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.FX_FACTORY_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.FX_ID_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.FX_NAMESPACE_PREFIX;
import static javafx.fxml.FXMLLoader.FX_NAMESPACE_VERSION;
import static javafx.fxml.FXMLLoader.FX_VALUE_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.IMPORT_PROCESSING_INSTRUCTION;
import static javafx.fxml.FXMLLoader.INCLUDE_CHARSET_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.INCLUDE_RESOURCES_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.INCLUDE_SOURCE_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.INCLUDE_TAG;
import static javafx.fxml.FXMLLoader.INITIALIZE_METHOD_NAME;
import static javafx.fxml.FXMLLoader.JAVAFX_VERSION;
import static javafx.fxml.FXMLLoader.LANGUAGE_PROCESSING_INSTRUCTION;
import static javafx.fxml.FXMLLoader.LOCATION_KEY;
import static javafx.fxml.FXMLLoader.NULL_KEYWORD;
import static javafx.fxml.FXMLLoader.REFERENCE_SOURCE_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.REFERENCE_TAG;
import static javafx.fxml.FXMLLoader.RELATIVE_PATH_PREFIX;
import static javafx.fxml.FXMLLoader.RESOURCES_KEY;
import static javafx.fxml.FXMLLoader.RESOURCE_KEY_PREFIX;
import static javafx.fxml.FXMLLoader.ROOT_TAG;
import static javafx.fxml.FXMLLoader.ROOT_TYPE_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.SCRIPT_CHARSET_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.SCRIPT_SOURCE_ATTRIBUTE;
import static javafx.fxml.FXMLLoader.SCRIPT_TAG;

public class UnicornFXMLLoader {
		...
		private boolean processValue(Class sourceType, String propertyName, String aValue)
			...
			if (sourceType == null && isTyped()) {
				...
				if (List.class.isAssignableFrom(type)
					...
					applyProperty(propertyName, null,
			...
			if (aValue.startsWith(ESCAPE_PREFIX)) {
				...
				if (aValue.startsWith(RELATIVE_PATH_PREFIX)) {
					...
					if (aValue.charAt(0) == '/') {
						...
						try {
							return new URL(UnicornFXMLLoader.this.location, aValue).toString();
						} catch (MalformedURLException e) {
							System.err.println(UnicornFXMLLoader.this.location + "/" + aValue);
						}
					}
				}
		...
		private void processValue() throws LoadException {
			...
			if (fx_id != null) {
				...
				if (idProperty != null) {
					...
					properties.putIfAbsent(idProperty.value(), fx_id);
				}
	...
	private class IncludeElement extends ValueElement {
		...
		public ResourceBundle resources = UnicornFXMLLoader.this.resources;
		public Charset charset = UnicornFXMLLoader.this.charset;
			...
			if (prefix == null) {
				switch (localName) {
					case INCLUDE_SOURCE_ATTRIBUTE:
						...
						source = value;
						break;
					case INCLUDE_RESOURCES_ATTRIBUTE:
						if (loadListener != null) {
							loadListener.readInternalAttribute(localName, value);
						}

						resources = ResourceBundle.getBundle(value, Locale.getDefault(),
								UnicornFXMLLoader.this.resources.getClass().getClassLoader());
						break;
					case INCLUDE_CHARSET_ATTRIBUTE:
						if (loadListener != null) {
							loadListener.readInternalAttribute(localName, value);
						}
						...
						break;
					default:
						super.processAttribute(null, localName, value);
						break;
				}
			} else {
				super.processAttribute(prefix, localName, value);
		...
		public Object constructValue() throws IOException {
			...
			if (source.charAt(0) == '/') {
				...
				if (UnicornFXMLLoader.this.location == null) {
					throw constructLoadException("Base location is undefined.");
				}

				location = new URL(UnicornFXMLLoader.this.location, source);
			}

			UnicornFXMLLoader fxmlLoader = new UnicornFXMLLoader(location, resources,
					builderFactory, controllerFactory, charset,
					loaders);
			fxmlLoader.parentLoader = UnicornFXMLLoader.this;

			if (isCyclic(UnicornFXMLLoader.this, fxmlLoader)) {
				throw new IOException(
						String.format(
								"Including \"%s\" in \"%s\" created cyclic reference.",
								fxmlLoader.location.toExternalForm(),
								UnicornFXMLLoader.this.location.toExternalForm()));
			}
	...
	private class PropertyElement extends Element {
		...
		@Override
		public boolean isCollection() {
			return (readOnly) && super.isCollection();
		}
	...
	private final LinkedList<UnicornFXMLLoader> loaders;

	private ClassLoader classLoader = null;
	private boolean staticLoad = false;
	private LoadListener loadListener = null;

	private UnicornFXMLLoader parentLoader;
	 ...
	 * @since JavaFX 2.1
	 */
	public UnicornFXMLLoader(URL location) {
		this(location, null);
	}
	 ...
	 * @since JavaFX 2.1
	 */
	public UnicornFXMLLoader(URL location, ResourceBundle resources) {
	 ...
	 * @since JavaFX 2.1
	 */
	public UnicornFXMLLoader(URL location, ResourceBundle resources, BuilderFactory builderFactory) {
	 ...
	 * @param charset
	 */
	public UnicornFXMLLoader(Charset charset) {
	...
	public UnicornFXMLLoader(URL location, ResourceBundle resources, BuilderFactory builderFactory,
		...
		this(location, resources, builderFactory, controllerFactory, charset,
				new LinkedList<UnicornFXMLLoader>());
	}
	...
	public UnicornFXMLLoader(URL location, ResourceBundle resources, BuilderFactory builderFactory,
	                         Callback<Class<?>, Object> controllerFactory, Charset charset,
	                         LinkedList<UnicornFXMLLoader> loaders) {
	...
	public boolean equals(Object obj) {
		if (obj instanceof UnicornFXMLLoader) {
			UnicornFXMLLoader loader = (UnicornFXMLLoader)obj;
	...
	private boolean isCyclic(
			UnicornFXMLLoader currentLoader,
			UnicornFXMLLoader node) {
	...
	private <T> T loadImpl(InputStream inputStream,
		...
		if (controller != null && root != null)
			injectRoot(controller, root);

		return (T)root;
	}

	private void injectRoot(@Nonnull Object controller, @Nonnull Object root) {
		try {
			for (Field field : getMutableInstanceFields(controller.getClass())) {
				if (!field.getType().isInstance(root))
					continue;

				if (field.getAnnotation(FXMLRoot.class) == null)
					continue;

				field.setAccessible(true);
				field.set(controller, root);
				return;
			}

			for (Method method : getInstanceMethods(controller.getClass())) {
				if (method.getParameterCount() != 1)
					continue;
				if (!method.getParameterTypes()[0].isInstance(root))
					continue;

				if (method.getAnnotation(FXMLRoot.class) == null)
					continue;

				method.setAccessible(true);
				method.invoke(controller, root);
				return;
			}
		} catch (Exception e) {
	...
	@Nonnull
	private static List<Method> getInstanceMethods(@Nonnull Class<?> clazz) {
	...
	@Nonnull
	private static List<Method> getAllMethods(@Nonnull Class<?> clazz) {
		return getAllMethods(clazz, Object.class);
	}

	@Nonnull
	private static List<Method> getAllMethods(@Nonnull Class<?> clazz, @Nonnull Class<?> topClass) {
	...
	private String constructFXMLTrace() {
		...
		for (UnicornFXMLLoader loader : loaders) {
		...
		if (defaultClassLoader == null) {
			...
			if (sm != null) {
				final ClassLoader callerClassLoader = (caller != null) ?
						...
						null;
				if (needsClassLoaderPermissionCheck(callerClassLoader, UnicornFXMLLoader.class.getClassLoader())) {
	...
	public static void setDefaultClassLoader(ClassLoader defaultClassLoader) {
		...
		UnicornFXMLLoader.defaultClassLoader = defaultClassLoader;
	}
	...
	private static <T> T loadImpl(URL location, ResourceBundle resources,
		...
		UnicornFXMLLoader fxmlLoader =
				new UnicornFXMLLoader(location, resources, builderFactory,
	...
	private static final class ControllerAccessor {
		...
		private void addAccessibleFields(final Class<?> type,
			...
			for (final Field field : fields) {
				...
				List<Field> list = controllerFields.computeIfAbsent(field.getName(), k -> new ArrayList<>(1));
				list.add(field);
